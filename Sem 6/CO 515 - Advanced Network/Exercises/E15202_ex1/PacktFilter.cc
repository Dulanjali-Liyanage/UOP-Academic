...
//************************************************************************
map<string, int> ipForwardingTable;
vector <int > serverMap;
int currentServer = 0;
//************************************************************************
...
void Forwarding::initialize(int stage)
{
	...
	//****************************************************************************************
	//1,3,4 is the node number in nodeinfo[i] --> i = 1,2, or 3
	serverMap.push_back(1);
	serverMap.push_back(3);
	serverMap.push_back(4);
	//****************************************************************************************
...
void Forwarding::receiveSignal(cComponent *src, simsignal_t id, cObject *obj)
{
    ...
    if (id == PacketInSignalId)
    {
        ...
        if (dynamic_cast<OF_Wrapper *>(obj) != NULL)
        {
            ...
            //Load Balancing
            //********************************************************************************************
            dst_ip = packet_in->getMatch().OFB_ARP_TPA; //destination ip address
            //********************************************************************************************
            EV <<"dest: " <<dst_ip.getDByte(3) << endl;
            EV <<"src: " <<src_mac << endl;
            EV <<"dest: " <<dst_mac << endl;

            if(dst_ip.getDByte(3) == 200){
            	if(ipForwardingTable.find(src_mac.str()) != ipForwardingTable.end()){
            		EV <<"Packet is in Table" << endl;
            		dst_mac = nodeInfo[serverMap[ipForwardingTable.find(src_mac.str())->second]].macAddress;
            	}
            	else{
            		ipForwardingTable[src_mac.str()] = currentServer;
            		EV <<"Adding flow to server: " <<currentServer << endl;
            		dst_mac = nodeInfo[serverMap[currentServer]].macAddress;
            ...
            //********************************************************************************************
            IPv4Address src_ip;
            src_ip = packet_in->getMatch().OFB_ARP_SPA; //source ip address

            EV <<"source ip: " <<src_ip << endl;

            if(src_ip.getDByte(3) == 1){
            	EV <<"Packet blocked" << endl;
            	EV <<"Resends the packet to source" << endl;
            	dst_mac = nodeInfo[0].macAddress;
            }
            //********************************************************************************************
