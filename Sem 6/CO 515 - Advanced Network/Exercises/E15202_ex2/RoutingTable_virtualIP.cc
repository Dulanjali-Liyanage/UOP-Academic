...
{
	//****************************************************************************
	//x1.x2.x3.y(y = 2,5,6) the ip addresses of the servers given by the configurator
	servers.insert(2);
	servers.insert(5);
	servers.insert(6);
	//****************************************************************************
...
//This function is used by a device to identify whether a packet arrived to the
//device is intended to him or not
//To achieve that it checks the destination address with the local ip address
bool RoutingTable::isLocalAddress(const IPv4Address& dest) const
{
    ...
    EV << "Checks destination ip with local ip" << endl;
    ...
    //*********************************************************************
    //x1.x2.x3.y from this type of ip takes the y(getDByte(3))
    int ip = ift->getInterface(1)->ipv4Data()->getIPAddress().getDByte(3);

    //If it is the server check the destination address x1.x2.x3.y
    //if y = 200 (which we take as the virtual ip)
    if(servers.find(ip) != servers.end() && dest.getDByte(3) == 200){
    	return true;
    }
    //*********************************************************************
